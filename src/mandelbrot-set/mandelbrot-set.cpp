...
namespace cw4 {
     ...
     * */
    Mandelbrot::Mandelbrot(double xMin, double xStep, double xMax,
                           double yMin, double yStep, double yMax):
        xMin(xMin), xStep(xStep), xMax(xMax),
        yMin(yMin), yStep(yStep), yMax(yMax) {
     ...
     * @param yMax The value of yMax.
     * */
    void Mandelbrot::setRange(double xMin, double xStep, double xMax,
                              double yMin, double yStep, double yMax) {
    ...
    void Mandelbrot::printRange(void) {
        std::cout << "Mandelbrot Set of: " << std::endl
            // Real parameters
                  << "x: [" << this->xMin
                  << ":" << this->xStep
                  << ":" << this->xMax << "], "
    ...
    void Mandelbrot::calcMBS(void) {
        ...
        for (unsigned int y = 0; y < Ny; y++) {
            ...
            for (unsigned int x = 0; x < Nx; x++) {
                // Initial values
                Complex c(this->xMin + x * this->xStep,
                          this->yMax - y * this->yStep);
                bool inSet = true;
                Complex z(0, 0);
